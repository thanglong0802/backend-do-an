...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query(value = "WITH RECURSIVE cte AS (SELECT id, created_at, created_by, updated_at, updated_by, name, parent_id FROM tbl_category WHERE id = :id UNION ALL SELECT c.id, c.created_at, c.created_by, c.updated_at, c.updated_by, c.name, c.parent_id FROM tbl_category c JOIN cte ON c.parent_id = cte.id) SELECT p.id, p.category_id, p.status, p.price, p.quantity, p.description, p.use, p.producer, p.where_production, p.name, p.created_at, p.created_by, p.updated_at, p.updated_by, c.id, c.name, c.parent_id FROM tbl_product p JOIN cte c ON p.category_id = c.id", nativeQuery = true)
